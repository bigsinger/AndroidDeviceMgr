...
public class DeviceManger {
    ...
    public void enableDeviceManager() {
        ...
        if (!devicePolicyManager.isAdminActive(componentName)) {
            ...
            intent.putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION, mContext.getString(R.string.dm_extra_add_explanation));
    ...
    // 禁止应用
    public void disableApp(String packageName, boolean disabled) {
        if (devicePolicyManager.isAdminActive(componentName)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                devicePolicyManager.setApplicationHidden(componentName, packageName, disabled);
            }
        } else {
            Toast.makeText(mContext, "请先激活设备", Toast.LENGTH_SHORT).show();
        }
